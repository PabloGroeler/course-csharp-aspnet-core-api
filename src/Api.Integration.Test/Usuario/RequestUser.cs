...
namespace Api.Integration.Test.Usuario
{
    public class RequestUser: BaseIntegration
    {
        ...
        public async Task DoCrud()
        {
            ...
            Assert.True(listJson.Where(r => r.Id == registroPost.Id).Count() == 1);
            ...
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
            jsonResult = await response.Content.ReadAsStringAsync();
            var registroSelecionadoId = JsonConvert.DeserializeObject<UserDto>(jsonResult);
            Assert.NotNull(registroSelecionadoId);
            Assert.Equal(registroSelecionadoId.Name, registroAtualizado.Name);
            Assert.Equal(registroSelecionadoId.Email, registroAtualizado.Email);

            // Delete
            response = await client.DeleteAsync($"{hostApi}users/{registroSelecionadoId.Id}");
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
